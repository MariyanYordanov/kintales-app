...
export function validateBio(bio) {
  ...
  if (trimmed.length > MAX_BIO_LENGTH) {
    return { valid: false, error: 'validation.bioTooLong' };
  }

  return { valid: true, error: null };
}

export function validateProfileForm({ fullName, bio, language }) {
  const nameResult = validateFullName(fullName);
  const bioResult = validateBio(bio);
  const langResult = validateLanguage(language);

  return {
    valid: nameResult.valid && bioResult.valid && langResult.valid,
    errors: {
      fullName: nameResult.error,
      bio: bioResult.error,
      language: langResult.error,
